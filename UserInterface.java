 ...
 * @co_author K.Shanthisai 2020501032
 * 
 * @version 3.1	2018-05-07 An implementation baseline for JavaFX graphics
 * 
 */
public class UserInterface {
	...
	**********************************************************************************************/
	...
	private int count =0;					// number of input lines
	...
	private Cell[][] cell;							// global cell variable

	private Board1 oddGameBoard = new Board1();		// The Board for odd frames of the animation
	private Pane oddCanvas = new Pane();			// Pane that holds its graphical representation
	
	private Board1 evenGameBoard =  new Board1();	// The Board for even frames of the animation
	...
	**********************************************************************************************/
	...
	private void loadImageData() {
		try {
			...
			File myObj = new File(str_FileName);
		    Scanner sc = new Scanner(myObj);
//		    System.out.println(str_FileName.length());
		    
		  
//	       FileReader input = new FileReader(myObj+".txt");
...
//	        	System.out.println("hello");
	        	...
	        	Scanner sc2 = new Scanner(inputline);
...
//	        }
	     // board size
	        int n = 100;
	        // creating live cells
	        cell = oddGameBoard.createBoard(n, l);
	        for (int m = 0; m < cell.length; m++){
				for (int o = 0; o < cell[0].length; o++) {
			
						if(cell[m][o].getStatus()) {
							Rectangle rectr = new Rectangle((m*cellSize),(o*cellSize),cellSize,cellSize);
							rectr.setFill(Color.BLACK);
							oddCanvas.getChildren().add(rectr);
	...
	public void runSimulation(){
		...
		Cell [][] cell2 = oddGameBoard.generateNextGeneration(cell);
		if (toggle) {
			toggle = false;
			oddCanvas.getChildren().clear();
			window.getChildren().remove(oddCanvas);
			for (int m = 0; m < cell2.length; m++){
				for (int o = 0; o < cell2[0].length; o++) {
			
						if(cell2[m][o].getStatus()) {
							Rectangle rect_angle = new Rectangle((m*cellSize),(o*cellSize),cellSize,cellSize);
			...
			Cell [][] cell3 = oddGameBoard.generateNextGeneration(cell2);
			toggle = true;
			cell = cell3;
			evenCanvas.getChildren().clear();
			window.getChildren().remove(evenCanvas);
			for (int m = 0; m < cell3.length; m++){
				for (int o = 0; o < cell3[0].length; o++) {
			
						if(cell3[m][o].getStatus()) {
							Rectangle rect_angle = new Rectangle((m*cellSize),(o*cellSize),cellSize,cellSize);
							rect_angle.setFill(Color.BLACK);
